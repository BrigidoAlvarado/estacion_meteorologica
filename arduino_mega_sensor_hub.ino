...
#define NODEMCU_RST_PIN  8   // Mega pin 8 → RST del NodeMCU
...
void setup() {
  ...
  digitalWrite(NODEMCU_RST_PIN, HIGH);  // HIGH = NodeMCU operando normal
...
// Despierta el NodeMCU bajando RST por 150ms y volviendo a HIGH
// El NodeMCU arranca desde cero, recibe el CSV y vuelve a dormir
void despertarNodeMCU() {
  Serial.println(F("  Despertando NodeMCU..."));
  digitalWrite(NODEMCU_RST_PIN, LOW);   // RST activo
  delay(150);                            // pulso mínimo de reset
  digitalWrite(NODEMCU_RST_PIN, HIGH);  // liberar RST
  delay(500);  // pequeña pausa para que el NodeMCU arranque su setup()
...
void enviarPromedio() {
  ...
  // Despertar NodeMCU ANTES de enviar — le damos tiempo de arrancar
  despertarNodeMCU();

  // Pequeña pausa para que el NodeMCU termine su setup() y abra Serial
  delay(1000);
